...
import com.ycw.auth.ops.system.mapper.SysMenuMapper;
...
import com.ycw.common.constant.MenuConstant;
...
import static com.ycw.common.exception.enums.GlobalErrorCodeConstants.INTERNAL_PARENT_MENU_NOT_CAN_LIST_TYPE;
import static com.ycw.common.exception.enums.GlobalErrorCodeConstants.INTERNAL_PARENT_MENU_ONLY_CAN_MENU_TYPE;
...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements SysMenuService {
    ...
    public List<SysMenu> selectMenuTree(Integer uid) {
        ...
        if (uid != 0) {
            List<Integer> menuIdList = sysRoleMenuService.getMenuIdByUserId(uid);
    ...
    private void verifyForm(SysMenu menu) {
        ...
        if (menu.getType() == MenuConstant.MenuType.CATALOG.getValue() ||
            ...
            if (parentType != MenuConstant.MenuType.CATALOG.getValue()) {
                throw new ServiceException(INTERNAL_PARENT_MENU_NOT_CAN_LIST_TYPE);
            }
            return;
        }
        //按钮
        if (menu.getType() == MenuConstant.MenuType.BUTTON.getValue()) {
            if (parentType != MenuConstant.MenuType.MENU.getValue()) {
                throw new ServiceException(INTERNAL_PARENT_MENU_ONLY_CAN_MENU_TYPE);
            }
        }
    }
}
